...
# DESC_03_03DV R version 1.01 - Biostat Global Consulting - 2023-06-04
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2023-06-04  1.00      Mia Yu          Original R package version
# 2024-10-04  1.01      Caitlin Clary   Use new variable (responded) to set DVs
#                                       to missing if the denominator is only
#                                       those who responded
# *******************************************************************************
...
DESC_03_03DV <- function(VCP = "DESC_03_03DV"){
  ...
  dat <- vcqi_read(
    paste0(VCQI_OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER, "_",
           DESC_03_COUNTER, ".rds"))
  ...
  for (v in seq_along(DESC_03_VARIABLES)){
    ...
    dat <- dat %>%
      mutate(
        tempvar1 = ifelse(
          # is.na(!!va) & stringr::str_to_upper(DESC_03_DENOMINATOR) == "RESPONDED",
    ...
    # the label for any option...we just call it 'missing')
  ...
  vcqi_log_comment(
    VCP, 3, "Global",
    paste0("Global value DESC_03_LVL_COUNT_", vcounter, " is ", lcounter-1))
  ...
  if (vcqi_object_exists("DESC_03_N_SUBTOTALS")){
    if (DESC_03_N_SUBTOTALS != 0){
      for (i in 1:DESC_03_N_SUBTOTALS){
        ...
        dat <- dat %>%
          mutate(
            tempvar2 = ifelse(
              ...
              # tempvar3 == 1 & str_to_upper(DESC_03_DENOMINATOR) == "RESPONDED",
              NA, tempvar2)
          ) #%>% select(-c(tempvar3))
        ...
        if (i == 1){
          dat <- dat %>%
            relocate(
              paste0("desc03_", vcounter, "_st", i),
              .after = paste0("desc03_", vcounter, "_", lcounter-1))
        }
        if (i > 1){
          dat <- dat %>%
            relocate(
              paste0("desc03_", vcounter, "_st", i),
              .after = paste0("desc03_", vcounter, "_st", i-1))
        }
    ...
    vcqi_log_comment(
      VCP, 3, "Global",
      paste0("Global value DESC_03_ST_COUNT_", vcounter, " is ", DESC_03_N_SUBTOTALS))

  } else {
    rm(list = c(paste0("DESC_03_ST_COUNT_", vcounter)), envir = .GlobalEnv) %>%
      suppressWarnings()
  } # end of check DESC_03_N_SUBTOTALS

  saveRDS(dat, file = paste0(VCQI_OUTPUT_FOLDER, "/DESC_03_", ANALYSIS_COUNTER,
