...
merge_data_for_MOV_dvs <- function(VCP = "merge_data_for_MOV_dvs"){
  ...
  for (d in seq_along(MOV_OUTPUT_DOSE_LIST)){
    dlist <- c(dlist, paste0("elig_",MOV_OUTPUT_DOSE_LIST[d],"_crude"),paste0("elig_",MOV_OUTPUT_DOSE_LIST[d],"_valid"),
               paste0("got_",MOV_OUTPUT_DOSE_LIST[d],"_crude"),paste0("got_",MOV_OUTPUT_DOSE_LIST[d],"_valid"),
  ...
  dat2 <- vcqi_read(paste0(VCQI_OUTPUT_FOLDER,"/RI_MOV_step07.rds")) %>% select(respid,got_visit,visitdate,all_of(dlist))
  dat <- left_join(dat,dat2,by = "respid",multiple = "all")

  # Generate variable "showed_card_with_dates"
  if ("card_date_count" %in% names(dat)){
    dat <- dat %>% select(-c(card_date_count))
  }
  ...
  for (d in seq_along(MOV_OUTPUT_DOSE_LIST)){
    ...
    dat <- dat %>% mutate(card_date_count = ifelse(!is.na(!!carddate), card_date_count + 1, card_date_count))
  } #end of MOV_OUTPUT_DOSE_LIST d loop

  dat <- dat %>% mutate(showed_card_with_dates = ifelse((card_date_count > 0) %in% TRUE, 1, 0))
  dat$showed_card_with_dates <- haven::labelled(dat$showed_card_with_dates, label = "Card Seen- Dates listed on Card")

  # Generate variable "has_card_with_dob_and_dose"
  dat <- dat %>% mutate(has_card_with_dob_and_dose =
                          ifelse(!is.na(dob_for_valid_dose_calculations) & showed_card_with_dates %in% 1,1,0))
  dat$has_card_with_dob_and_dose <- haven::labelled(dat$has_card_with_dob_and_dose,
  ...
  for (vc in seq_along(type)){
    for (d in seq_along(MOV_OUTPUT_DOSE_LIST)){
      ...
      dat$tempvar1 <- haven::labelled(dat$tempvar1, label = "Correct no dose: Not eligible & did not receive it")
      dat$tempvar2 <- haven::labelled(dat$tempvar2, label = "Invalid dose: Not eligible & rec'd it")
      dat$tempvar3 <- haven::labelled(dat$tempvar3, label = "MOV dose: Eligible & did not receive it")
      dat$tempvar4 <- haven::labelled(dat$tempvar4, label = "Correct valid dose: Eligible & rec'd it")

      names(dat)[which(names(dat) == "tempvar1")] <- paste0("correct_nodose_",MOV_OUTPUT_DOSE_LIST[d],"_",type[vc])
      names(dat)[which(names(dat) == "tempvar2")] <- paste0("invalid_",MOV_OUTPUT_DOSE_LIST[d],"_",type[vc])
      names(dat)[which(names(dat) == "tempvar3")] <- paste0("mov_",MOV_OUTPUT_DOSE_LIST[d],"_",type[vc])
      names(dat)[which(names(dat) == "tempvar4")] <- paste0("correct_validdose_",MOV_OUTPUT_DOSE_LIST[d],"_",type[vc])
  ...
  dat <- dat %>% select(c(ES01AA, ID02AB, ID02AD, ID02AF, ID02AH, ID02AIid, all_of(VCQI_LEVEL4_SET_VARLIST), all_of(dlist),
                          dob_for_valid_dose_calculations, ES06AA, all_of(keep_dose_card_date_list), all_of(starts_with("ES08AA")),
                          level1id, level2id, level3id, stratumid, clusterid, respid, HH02, HH04, psweight, got_visit, visitdate,
                          card_date_count, showed_card_with_dates, has_card_with_dob_and_dose, all_of(starts_with("correct_nodose_")),
                          all_of(starts_with("invalid_")), all_of(starts_with("mov_")), all_of(starts_with("correct_validdose_"))))
  ...
  dat <- dat %>% filter(got_visit %in% 1) %>% select(-c(got_visit))
