#' Export analysis table(s) to Excel with styles for ES_STUD_03 long table only
#'
...
# export_ALT_to_excel R version 1.00 - Biostat Global Consulting - 2023-10-02
# *******************************************************************************
# Change log

# Date 			  Version 	Name			      What Changed
# 2023-10-02  1.00      Mia Yu          Original R version
# *******************************************************************************

export_ALT_to_excel <- function(indicator, tablename = NULL, sheet = NULL, VCP = "export_ALT_to_excel"){
  ...
  format1col <- which(formatnum %in% 1) + 2
  format2col <- which(formatnum %in% 2) + 2
  format3col <- which(formatnum %in% 3) + 2
  format4col <- which(formatnum %in% 4) + 2
  format5col <- which(formatnum %in% 5) + 2
